...
export default function AdminCallsPage() {
	...
	useEffect(() => {
		setLoading(true);
		fetch(`/api/admin/calls?page=${page}&limit=50`)
			.then((res) => res.json())
			.then((data) => {
				if (data.calls) {
					setCalls(data.calls);
				...
				<div className="px-4 lg:px-6">
					<div className="mb-6">
						...
						<p className="text-muted-foreground">
							View calls across all companies
						</p>
					</div>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								All calls handled by AI dispatchers across the platform
							</CardDescription>
						</CardHeader>
						<CardContent>
							{loading ? (
								...
								<div className="flex min-h-[400px] flex-col items-center justify-center text-muted-foreground">
									...
									<p className="mt-1 max-w-sm text-center text-sm">
										Calls will appear here once companies start receiving them.
									</p>
								</div>
							) : (
								<>
											...
											{calls.map((call) => (
												<TableRow key={call.id}>
													...
													<TableCell>
														{call.company_id ? (
															<Link
																href={`/admin/companies/${call.company_id}`}
